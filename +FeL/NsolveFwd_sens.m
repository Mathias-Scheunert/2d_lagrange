function u = NsolveFwd_sens(sol, fe, verbosity)
    ...
    if isfield(sol, 'dirichlet')
        ...
        u_red1 = sol.A \ sol.b(:,1);
        u_red2 = sol.A \ sol.b(:,2);
        ...
        u{1} = zeros(fe.sizes.DOF, size(sol.b(:,1), 2));
        u{1}(sol.dirichlet.DOF_req,:) = u_red1;
        u{1}(sol.dirichlet.bnd_DOF,:) = repmat(sol.dirichlet.val, 1, size(sol.b(:,1), 2));
        u{2} = zeros(fe.sizes.DOF, size(sol.b(:,1), 2));
        u{2}(sol.dirichlet.DOF_req,:) = u_red2;
        u{2}(sol.dirichlet.bnd_DOF,:) = repmat(sol.dirichlet.val, 1, size(sol.b(:,1), 2));
