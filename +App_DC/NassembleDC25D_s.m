function [fe, sol, FT_info, bnd_reduc] = NassembleDC25D_s(mesh, param, fwd_params, verbosity)
    ...
    [A_DivGrad, TS] = FeL.assembleStiff(fe, mesh, param, verbosity); %%%%%%%%%%%%%%%%
    [A_Mass, TM] = FeL.assembleMass(fe, mesh, param, verbosity);     %%%%%%%%%%%%%%%%
    ...
    if verbosity
        fprintf('Assemble linear system ... ');
    end
    sol = cell(FT_info.n, 1);
    bnd_reduc= [];
    % TODO: avoid storing redundand information.
    % T2, A2 are not treated with bnd
    for ii = 1:(FT_info.n) %ks
        % Set up system matrix
        sol{ii}.A = A_DivGrad + FT_info.k(ii)^2 * A_Mass;
        %sol{ii}.A2 = sol{ii}.A;
        % set up derivative of system Matrix
        temp = TM  .* FT_info.k(ii)^2;
        sol{ii}.TA = TS + temp;
        %sol{ii}.TS = TS;
        %sol{ii}.TMk = temp;
        % Set up rhs.
        ...
        bnd.k = FT_info.k(ii);
        if eq(ii,1)
            [sol{ii}, bnd_reduc] = FeL.NtreatBC_dir(fe, mesh, sol{ii}, bnd);
        else
            [sol{ii}, ~] = FeL.NtreatBC_dir(fe, mesh, sol{ii}, bnd);
        end
    end
