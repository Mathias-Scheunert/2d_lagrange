function bnd = getBndDOF(fe, mesh, bnd)
    ...
    assert(any(strcmp(mesh.type, {'cube', 'rhomb', 'gmsh_create', 'gmsh_load'})), ...
    ...
    [name_match, name_bnd_in_mesh] = ismember(bnd.name, ...
                                        mesh.bnd_edge_part_name);
    assert(all(name_match), ...
        ['Missmatch between boundary part names stored in the mesh ', ...
        'struct and given by the bnd struct. Please make sure that ', ...
        'all boundary parts of the mesh are related to an appropriate ', ...
        'boundary condition while setting up the DRIVE_ script.']);
    
    % Sort information given bnd to match the order of mesh.
