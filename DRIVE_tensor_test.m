...
[fe, sol, FT_info, bnd_reduc] = App_DC.NassembleDC25D_s(mesh, param, fwd_params);
fprintf('.');
fprintf('\n');
...
% Initialisieren.
rng(0815);
d_param = randn(size(param));
d_param = d_param / norm(d_param) * 4.5e-1;
h = logspace(0, -8, 51);
n_h = length(h);
...
for ii = 1:n_h
    ...
    param_tt = param + h(ii)*d_param;
    [~, tmp, ~, ~] = App_DC.NassembleDC25D_s(mesh, param_tt, fwd_params);
    sol_tt(:, ii) = tmp;
    fprintf('.');
end
fprintf('\n');
...
for kk = 1%:FT_info.n
    ...
    for ii = 1:n_h
        ...
        err_tt_2(ii) = norm(sol_tt{kk, ii}.A - ...
                           (sol{kk}.A + ttv(sol{kk}.TA, h(ii)*d_param, 3)), ...
                            'fro');
        % Nur Tensor(en) verwenden.
        A_TA = ttv(sol{kk}.TA, param + h(ii)*d_param, 3);
        A_TA0 = ttv(sol{kk}.TA, param, 3);
        err_tt_3(ii) = norm(A_TA - (A_TA0 + ttv(sol{kk}.TA, h(ii)*d_param, 3)), ...
                            'fro');
    end
